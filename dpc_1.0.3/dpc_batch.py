...
def load_scan_from_mds(scan_id, scan_header_id = 0):

    hdrs = DataBroker(scan_id=scan_id)
    hdr = hdrs[scan_header_id]
...
def get_ref_from_mds(scan, first_image = 0):
    if scan is None:
        return
...
def save_results(a, gx, gy, phi, rx, ry, save_path, scan_number, save_pngs = True):

    save_filename = os.path.join(save_path,'S{0}'.format(scan_number))
...
def main():
    ...
    scan_range, scan_numbers, every_nth_scan, get_data_from_datastore, data_directory, \
        read_params_from_datastore, parameter_file, processes, scan_header_index, file_format, \
        file_store_key = parse_script(script_file)
    ...
    for i_scan in range(n_scans):
        ...
        if get_data_from_datastore:
            ...
            try:
                mds_scan = load_scan_from_mds(i_scan, scan_header_id=scan_header_index)
            except Exception as ex:
                print('Filestore load failed (datum={}): ({}) {}'
                    ''.format(i_scan, ex.__class__.__name__, ex))
                raise
            ...
            if read_params_from_datastore == 1:
                ...
                dpc_settings['scan'] = pyramid_scan
        ...
        save_results(a, gx, gy, phi, rx, ry, data_directory, calc_scan_numbers[i_scan])
